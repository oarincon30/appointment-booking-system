...
public interface AppointmentRepository extends JpaRepository<Appointment, Long>, JpaSpecificationExecutor<Appointment> {
     ...
     * Checks conflicts for same dentist (if provided) OR same patient.
     */
    @Query("""
    SELECT CASE WHEN COUNT(a) > 0 THEN TRUE ELSE FALSE END
    FROM Appointment a
    WHERE a.status IN (com.appointmentbooking.domain.AppointmentStatus.SCHEDULED,
                       com.appointmentbooking.domain.AppointmentStatus.CONFIRMED)
